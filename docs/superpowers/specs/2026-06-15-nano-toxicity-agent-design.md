...
当前方案面向课程作业原型，重点是可运行、可解释、可演示。它能体现智能体体验，但还不是完整意义上的自主智能体系统。

### 10.1 当前方案不足

主要不足：

- 智能体能力较轻量：当前系统通过预测函数、解释函数、建议函数和可选 LLM 输出组合出智能体体验，但没有引入完整 Agent 框架，也没有复杂的工具规划、任务分解和多轮状态管理。
- 文献处理较静态：当前方案使用本地固定文献摘要库，优点是可控、可追溯，但只能覆盖预先整理好的论文，不支持从大量 PDF 或网页中动态检索证据。
- 模型能力受数据限制：公开纳米毒性数据通常规模有限、字段不统一、实验条件差异大，模型性能可能受数据质量影响。
- 解释能力以特征重要性为主：随机森林特征重要性能说明整体影响因素，但对单个样本的局部解释不够细。
- 体外到体内转化不足：本方案预测体外细胞毒性，不覆盖体内药代动力学、器官蓄积、免疫反应和长期毒性。
- 数据来源整合不足：第一版通常只选择一个主要数据集训练，尚未完成 eNanoMapper、caNanoLab 和其他公开数据集的统一 schema 对齐。
- LLM 不能保证科学正确性：LLM 负责表达和总结，但如果上下文不足，仍可能产生不严谨解释，因此必须限制其输入来源和输出边界。

### 10.2 后续改进方向

后续可按优先级逐步增强：

1. 引入 Agent 框架，实现真正的工具调用型智能体
   - 可选技术：LangChain、LlamaIndex、AutoGen 或自研轻量 tool-calling loop。
   - 工具集合：`toxicity_predictor`、`literature_search`、`feature_explainer`、`experiment_advisor`、`report_generator`。
   - 目标：让智能体能根据用户问题自动选择工具，而不是固定执行一条预测流程。

2. 使用 RAG 升级文献处理
   - 将核心论文 PDF、摘要、引言、方法和结论切分成文本块。
   - 使用 embedding 模型生成向量。
   - 使用 FAISS、Chroma 或 LanceDB 构建本地向量库。
   - 用户追问时先检索相关文献片段，再交给 LLM 生成回答。
   - 目标：让文献依据从“固定摘要”升级为“可检索证据片段”。

3. 增强模型解释能力
   - 引入 SHAP 或 LIME 做单样本局部解释。
   - 输出“为什么这个样本被判为中毒”的逐项贡献。
   - 对比全局特征重要性和当前样本局部解释。
   - 目标：提高预测结果的可信度和答辩说服力。

4. 扩展数据集与统一 schema
   - 整合 eNanoMapper、caNanoLab、NanoTox、PubVINAS 等数据源。
   - 建立统一字段：材料属性、实验条件、细胞信息、毒性终点。
   - 记录每条样本的数据来源和清洗规则。
   - 目标：提升模型泛化能力和数据来源可信度。

5. 从体外毒性扩展到更完整的安全性评估
   - 增加氧化应激、炎症因子、IC50、ROS 等多任务预测。
   - 后续可探索 PBPK 或体内毒性数据，但这超出第一版课程原型范围。
   - 目标：从单一细胞活力预测扩展为纳米材料安全性评估助手。

6. 增加实验设计建议能力
   - 根据预测结果推荐补充细胞系、剂量梯度、暴露时间和检测方法。
   - 对高风险样本给出“降低剂量、改变表面修饰、增加正常细胞对照”等建议。
   - 目标：让系统不仅给结论，也能辅助下一步实验设计。
...
```text
Streamlit UI + scikit-learn 模型 + 本地文献摘要库 + 模板/LLM 解释
```

第二版升级智能体能力：

```text
Agent 框架 + 工具调用 + 多轮上下文 + 预测/解释/文献/报告工具
```

第三版升级知识依据：

```text
PDF 文献库 + RAG 检索 + 引用片段 + 可追溯回答
```

第四版升级科学建模：

```text
多数据源整合 + SHAP/LIME 解释 + 多任务毒性终点 + 更严格评估
```
